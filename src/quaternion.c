...
int main()
{
  struct Quaternion q1 = Quaternion_new(1, 2, 3, 2);
  struct Quaternion q2 = Quaternion_new(1, 2, 3, 5);
  q1 = Quaternion_mulScalar(q1, 10);
  struct Quaternion q3 = Quaternion_addQuaternion(q1, q2);

  printf("\nq1: %f, %f, %f, %f", q1.x, q1.y, q1.z, q1.w);
  printf("\nq2: %f, %f, %f, %f", q2.x, q2.y, q2.z, q2.w);
  printf("\nq3: %f, %f, %f, %f", q3.x, q3.y, q3.z, q3.w);
  printf("\n\n");

  float rotmat[3][3];
  Quaternion_toRotationMatrix(q3, rotmat);
  for (int i = 0; i < 3; i++)
  {
    for (int j = 0; j < 3; j++)
    {
