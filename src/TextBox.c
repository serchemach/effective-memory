...
void UpdateTextBox(SDL_Renderer* renderer, TextBox* tBox, int mousePosX, int mousePosY, int mouseLeftDown, char lastChar, int backspacePressed)
{
    ...
    if (tBox->isUsed == 1)
    {
        if (tBox->brightness > 51)
            tBox->brightness -= 1;
        else if (tBox->brightness < 50)
            tBox->brightness += 0.3;
    }
    else 
    {   
    ...
    switch(tBox->isUsed)
    {
        case 1:
            ...
            else if (((lastChar <= 57 && lastChar >= 48) || (lastChar == '.' && tBox->hasADot == 0)) && tBox->curTextSize < TBOX_TEXT_LENGTH)
            {
                ...
                if (tBox->curTextSize == 1 && tBox->text[0] == '0' && (lastChar != '.' || tBox->hasADot == 1))
                    tBox->text[0] = lastChar;
                else if (tBox->curTextSize == 2 && tBox->text[1] == '0' && tBox->text[0] == '-' && (lastChar != '.' || tBox->hasADot == 1))
